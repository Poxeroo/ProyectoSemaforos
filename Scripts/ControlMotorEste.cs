...
public class ControlMotorEste : MonoBehaviour
{
    [Range(0f, 10f)]
    public float VelocidadMotorEste = 5f;
    ...
    void Start()
    {
        ...
        rigidB2D.velocity = Vector2.left * VelocidadMotorEste;// el velocity es como se reconoce la velocidad de rogidbody2d 
    }
    ...
    void OnTriggerEnter2D(Collider2D colision)//sobre escribimos esta funcion osea que obligatoria mente se debe llamar asi
    {
        ...
        {
            rigidB2D.velocity = Vector2.left * VelocidadMotorEste;
        }

        if (colision.gameObject.tag == "Stop")
        {
            rigidB2D.velocity = Vector2.left * 0;
        }
